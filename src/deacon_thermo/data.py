...
DB.add(
    Species(
        ...
        -43.0,
        285.46,
        (64.845, 0.3103, -3.8724),
        {"Cu": 1, "Cl": 2},
        EST,
        "S/Cp は分光定数からの統計力学計算 (±4 J/mol/K、docs/cucl2g_statmech.md。"
        "同一コードで CuCl(g) の JANAF 値を -0.24 J/mol/K で再現)。"
        "dHf は要検証のまま (JANAF/WebBook に気相熱化学なし、"
        "Wächter & Schäfer, ZAAC 471 (1980) 38 要取得)。実測寿命 9600 h"
        "(Feng 2015 の実条件 GHSV 450) からの下限は >= -52 (理想) / -49 (較正) で"
        "現行値 -43 は整合。支配蒸気種なので揮発の絶対値はこの dHf に直結",
    )
)
...
LNOCL_PARAMS = {
    ...
    "Ho": (
        -35.04, +4.3, FAIR,
        "dH: Yang 2022, Inorg. Chem. 61, 7590, Table 2, p.7593 (-35.04±0.70)。"
        "dS: 未測定のため実測 5 元素 (La/Pr/Nd/Sm/Gd, Koch 系フレーム) の平均",
    ),
    "Er": (
        -37.83, +4.3, FAIR,
        "dH: Yang 2022, 同 Table 2 (-37.83±0.69)。dS: 実測 5 元素の平均",
    ),
    "Y": (
        -38.16, +9.4, FAIR,
    ...
    # Ce/Eu/Dy は比-内挿法による推定 (EST):
    # Burns 1998 (JAC 265, 146, Fig.2) の方法論 -- 比 dHf(LnCl3)/dHf(LnOCl) が
    # Shannon 半径に線形 -- を、当 DB の実測 7 元素 (La/Pr/Nd/Sm/Gd/Ho/Er) で
    # 再実装して内挿 (フィット残差 std 0.0031 -> DH で ±3 kJ/mol)。
    # dS は実測 5 元素の平均 +4.3 J/mol/K。
    "Ce": (
        -55.7, +4.3, EST,
        "比-内挿法 (2026-08-10, ±3)。酸化雰囲気ではどのみち CeO2 に行く (軸A)",
    ),
    "Eu": (
        -53.9, +4.3, EST,
        "比-内挿法 (2026-08-10, ±3)。独立検証: Burns 1998 (JAC 265, 146, p.150) の"
        "実測 dHf(EuOCl) = -903.5±3 を Gd オフセット (-14.5、Morss 系補助データとの"
        "流儀差) で補正すると -55.5 となり 1.6 kJ/mol で一致。"
        "なお Eu(II) が絡む場合は別扱いが要る (軸A)",
    ),
    "Dy": (
        -34.2, +4.3, EST,
        "比-内挿法 (2026-08-10, ±3)。旧既定値 -58 による系列順位の"
        "アーティファクトを解消 (隣接実測 Ho -35.04 と整合)",
    ),
}
