...
要点
----
DH_OXYCHLORIDE は系列を通して共通なので、振っても全 Ln の chloride_margin が
同じ量だけ平行移動する。したがって**系列内の順位は不変**で、動くのは
各 Ln の絶対判定（余裕の符号）だけ。これが CLAUDE.md の
「系列内の差は絶対値より信頼できる」の定量的な根拠になる。
