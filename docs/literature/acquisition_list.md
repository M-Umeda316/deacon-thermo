...
## 未入手文献の代替戦略(2026-08-09。入手経路の見通しが立たない場合)

入手できなくてもプロジェクトが止まらないよう、影響と回避策を整理:

| 文献 | 無い場合の影響 | 回避策 |
|---|---|---|
| **S2: Ruthven & Kenney 1968**(W(Cu-LnCl₃) の唯一のデータ) | Ln 添加の Cu 活量への効果が定量できない(希釈効果のみ) | **W(Cu-LnCl₃) = 0 を基準に ±10 kJ/mol の感度帯で運用**し、結論には帯を付ける。最終的には自前実験(系列比較)が経験較正になる。Kenney 1975 / Villadsen 1978 のレビュー再録が取れれば代替可 |
| **S6: Wächter & Schäfer 1980**(CuCl₂(g)) | 揮発の絶対値が EST のまま | 二段構え: (1) CuCl₂(g) の S°/Cp は公開の分光定数(直線 D∞h、振動数)から**統計力学で確定**できる。(2) 残る ΔHf は「実測寿命 ≥9600 h」からの**逆算で下限を拘束**(volatility.py の逆問題)。コード側で実施可能 |
| S5: Thiel & Seifert 1988(Sm 複塩) | — | **入手済みの Seifert 2002 レビュー(0812)の転記で代替**(優先順位 2) |
| A9(Y): Seifert & Büchel 1998 | Y/Ho 対照の熱力学裏打ちが片側のみ | 当面 Ho 側(0815)だけで進め、Y は「複塩データ無し」と明記。Lu 2014 の CALPHAD(こちらも有料)が取れれば代替 |
| A6: Knacke 編纂 | — | Jacob 2016(OA、取得済み)経由で実質カバー済み |
| A2 Blachnik / A8 Bloom / A10-A12 | 独立クロスチェックが薄くなるだけ | 保留で支障なし |

なお個人でも使える入手経路として、国会図書館の遠隔複写サービス(所属不要、
1 論文数百円+送料、Web 申込)は S2/S6 のような古い Elsevier/Wiley 誌に有効。
急がないが、S2 と S6 の 2 本だけは申し込む価値がある。
