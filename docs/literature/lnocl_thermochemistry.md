...
## 追記(2026-08-05): Gibson ら 2025 プレプリント入手・検証済み

SSRN プレプリント(`docs/literature/pdf/gibson2025_reocl_heat_capacity_ssrn.pdf`)を
入手し、§1.4 の [抄録] 値を全文で確認した。**要取得 #4 は実質解消**(査読版 JCT との
差分確認のみ残る)。

- **規格化は確定**: 「oxides and chlorides からの生成」は質量保存より
  1/3 Ln₂O₃ + 1/3 LnCl₃ → LnOCl(LnOCl 1 mol あたり)。§1.4 の「要検証」は解消。
- **298.15 K の反応量(Tables 13–15)**:
  NdOCl: ΔH_r = **−60.5 ± 0.72** kJ/mol, ΔS_r = **−7.1** J/mol/K, ΔG_r = −58.4 kJ/mol。
  YOCl: ΔH_r = −38.2 ± 0.58, ΔS_r = +9.4, ΔG_r = −41.0。
  TmOCl: ΔG_r(298) = −11.1(2D SmSI 型)。
- **含意 1**: 現行採用値 (DH_OXYCHLORIDE, DS_OXYCHLORIDE) = (−58, −8) は
  Nd の実測 (−60.5, −7.1) とほぼ一致。Sm(Knacke)アンカーと独立な熱量測定が
  同じ組を支持する。
- **含意 2**: Y の (−38.2, +9.4) は Nd と 22 kJ/mol 違い、ΔS は符号も逆。
  §G2(系列一定仮定の破れ)の直接の実証。Y を含む系列比較では element 別の
  補正が必要。
- **注意**: 本論文の絶対エントロピー(例: S°₂₉₈(NdOCl) = 77.19 J/mol/K)は
  格子項のみで核・磁気項を除外している。Barin 系補助データ(磁気項込み)と
  混ぜて使ってはいけない。**使うのは反応量 ΔH_r / ΔS_r(磁気項が相殺)のみ。**
